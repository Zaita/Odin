        ...
        <div className="flex mr-auto">
          ...
          <div className="flex ml-auto" style={{paddingTop: "10px"}}>
            ...
            {topMenuItems.map && topMenuItems.map((item, index) => {
              if (item.length == 3) {
                return (<div key={index} className="w-24 table-cell text-center"><Link href={route(item[1], item[2])}>{item[0]}</Link></div>)
              } else { // length == 2
                return (<div key={index} className="w-24 table-cell text-center"><Link href={route(item[1])}>{item[0]}</Link></div>)              
              }
            })}                 
            </div>
          </div>
