...
:depth: 4
```
This page will cover some basic configuration of Odin.
...
The admin panel has the following pages pre-configured with Odin.
* Pages - Not used by Odin, SilverStripe default.
* Files - Not used by Odin, SilverStripe default.
* Reports - Pre-built reports about Odin usage.
* Security - User, Group and Role configuration.
* Archive - Not used by Odin, SilverStripe default.
* Audit - An audit log of actions undertaken within Odin by users and admins.
* Jobs - Cron and scheduled jobs for Odin. When Odin needs to email users, it will create a job here.
* Questionnaires - Configuration for the dashboard, pillars, questionnaires and tasks (including risk assessments)
* Questionnaires Submissions - List of all questionnaire submissions in Odin.
* Security Components - Defined security control catalogues and controls with associated risk ratings.
* Service Inventory - List of services/systems that have a certification and accreditation issued by Odin.
* Task Submissions - List of all tasks that have been generated by Odin as part of a submission.
...
_Note: While the CMS supports adding more dashboards, Odin will only recognise and use one. Creating anymore than one dashboard may have unintended negative impacts_
...
An example `action` question is one that asks the user "Will you require any firewall changes for this delivery?". The options presented to the user will be `Yes` or `No`. If the user selects `Yes`, then Odin will navigate to the next question which will be an `input` question asking for details of the firewall rule changes. If the user answers `No` then Odin will navigate over the next `input` question to the next relevant question.
...
7. Special Field Type - (e.g., product name if type is text) - This is a special field indicator for Odin. It's used to fill in special fields as part of the submission.
...
#### Field Types
Odin supports the following field types:
...
#### Special Fields
Odin has special fields that are used for nicely cataloging and displaying information. The following special fields are currently configured in Odin:
...
## Adding a New Task
You can add as many tasks as you would like to Odin, then link these to questionnaires and questions following the instructions above.
